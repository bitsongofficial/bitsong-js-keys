...
export function getNewWalletFromSeed(mnemonic: string): Wallet {
  ...
  const bitsongAddress = getBitsongAddress(publicKey)
  return {
...
export function getBitsongAddress(publicKey: Buffer): string {
  const message = CryptoJS.enc.Hex.parse(publicKey.toString('hex'))
  const address = CryptoJS.RIPEMD160(CryptoJS.SHA256(message) as any).toString()
  const bitsongAddress = bech32ify(address, `bitsong`)

  return bitsongAddress
}
...
function deriveKeypair(masterKey: bip32.BIP32): KeyPair {
  const bitsongHD = masterKey.derivePath(hdPathAtom)
  const privateKey = bitsongHD.privateKey
